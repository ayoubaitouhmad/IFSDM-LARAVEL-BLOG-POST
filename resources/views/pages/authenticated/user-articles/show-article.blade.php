...
@section('content')
    <div class="container mx-auto px-6 py-12">
        ...
        <form action="{{ route('user.articles.update' , $article) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-2xl shadow-lg space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea
                    ...
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"  >{{ old('content', $article->content) }}</textarea>
                @error('content')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
            </div>

            <!-- Status -->
            <div>
                <x-form.select required name="status" label="status" value="{{ $article->status }}" :options="$statusOptions" />
